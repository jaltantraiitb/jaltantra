...
//			  String salt = "salt12345"; // Extra text to be used while password encryption 
...
//            String newpassword = encrypted_newpwd.toString();
			...
			PreparedStatement update = conn.prepareStatement("update jaltantra_users set password=? where username=?");
			...
			if(r1.next() && password.equals(r1.getString(8))) {
			...
			// If user does not exits, execute the insert command using executeUpdate() to make changes in database
				...
				PrintWriter out = response.getWriter();
				  ...
				  println("alert('Error: Your username or current password is wrong, please try again');"); 
